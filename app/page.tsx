    ...
    >
      <div style={{ background: "#fff", borderRadius: 16, padding: 24, width: 280, border: "0.5px solid #ddd" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={onClose} style={{ flex: 1, padding: "9px 0", borderRadius: 8, border: "0.5px solid #ccc", background: "#f5f5f5", fontSize: 14, cursor: "pointer" }}>
            취소
          </button>
          <button
          ...
          >
            ...
            {!isEmpty ? (
              <>
                ...
                {s.pending && (
                  <span style={{ fontSize: 8, color: isConfirming ? "#E24B4A" : isSupport ? "#7F77DD" : "#888", opacity: 0.8 }}>
                    대기
                  </span>
                )}
              </>
...
function GroupCard({ group, gi, isMaster, myStatus, accountName, onApply, onCancel, onLeave, onAccept, onReject }: {
  ...
  return (
    <div style={{ background: "#fff", border: "0.5px solid #e5e5e5", borderRadius: 12, padding: "12px 14px", marginBottom: 10 }}>
      ...
      <Slots
        group={group} gi={gi} canApply={canApply} accountName={accountName}
        onApply={onApply} onCancel={onCancel} onLeave={onLeave}
      />
...
export default function App() {
    ...
    const timer = setInterval(() => {
      setParties((prev) =>
        prev.filter((p) => {
          ...
          return new Date(p.date).getTime() + 60 * 60 * 1000 > Date.now();
        })
      );
    }, 60 * 1000);
  ...
  function updateParties(fn: (prev: Party[]) => Party[]) {
    setParties((prev) => {
      const next = fn(prev);
      localStorage.setItem("loa_parties", JSON.stringify(next));
      return next;
    });
  }
  ...
  useEffect(() => {
    ...
    const savedParties = localStorage.getItem("loa_parties");
    if (savedParties) {
      try { setParties(JSON.parse(savedParties)); } catch {}
    }
  }, []);

  // parties 바뀔 때마다 저장
  useEffect(() => {
    localStorage.setItem("loa_parties", JSON.stringify(parties));
  }, [parties]);
  ...
  function saveParties(updated: Party[]) {
    localStorage.setItem("loa_parties", JSON.stringify(updated));
  }
  ...
  function deleteParty(partyId: number) {
    updateParties((prev) => prev.filter((p) => p.id !== partyId));
    setScreen("list");
    showToast("파티를 삭제했습니다.");
  }

  function updateDate(partyId: number, date: string) {
    updateParties((prev) => prev.map((p) => p.id !== partyId ? p : { ...p, date }));
  }
  ...
  function confirmApply(charName: string) {
    if (!popup) return;
  ...
  function cancelApply(partyId: number, gi: number) {
    updateParties((prev) =>
      prev.map((p) =>
        p.id !== partyId ? p : {
          ...p,
  ...
  function leave(partyId: number, gi: number) {
    updateParties((prev) =>
      prev.map((p) =>
        p.id !== partyId ? p : {
          ...p,
  ...
  function accept(partyId: number, gi: number, ai: number) {
    updateParties((prev) =>
      prev.map((p) => {
        ...
        return {
          ...
          groups: p.groups.map((g, i) => {
            ...
            if (app.role === "서폿" && g.members.some((m) => m.role === "서폿")) {
              showToast("서폿 자리가 찼습니다!"); return g;
            }
            if (app.role === "딜러" && g.members.filter((m) => m.role === "딜러").length >= 3) {
              showToast("딜러 자리가 찼습니다!"); return g;
            }
  ...
  function reject(partyId: number, gi: number, ai: number) {
    updateParties((prev) =>
      prev.map((p) =>
        p.id !== partyId ? p : {
          ...p,
    ...
    showToast("거절했습니다.");
      ...
      function createParty() {
        ...
        setParties((prev) => [
          {
        id: Date.now(), raid: form.raid, masterId: myName, masterName: myName,
        partyCount: form.partyCount, date: form.date,
  ...
  return (
    <div style={{ width: "100%", maxWidth: "100vw", padding: "0 16px", boxSizing: "border-box", paddingBottom: 80, fontFamily: "sans-serif" }}>
      {toast && (
      ...
      {popup && (
        <ApplyPopup role={popup.role} onConfirm={confirmApply} onClose={() => setPopup(null)} />
      )}

      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "16px 16px 8px" }}>
        {screen !== "list" && (
          <button onClick={() => setScreen("list")} style={{ background: "none", border: "none", fontSize: 18, cursor: "pointer", padding: 0 }}>
            ←
          </button>
        )}
              ...
              {filteredParties.map((p) => {
                ...
                return (
                  <div
                    ...
                    style={{ background: "#fff", border: "0.5px solid #e5e5e5", borderRadius: 14, padding: "12px 14px", cursor: "pointer" }}
                  >
      ...
      {screen === "create" && (
        <div style={{ padding: "0 16px" }}>
          <div style={{ background: "#fff", border: "0.5px solid #e5e5e5", borderRadius: 14, padding: 16 }}>
              ...
              >
                {Object.entries(RAIDS).map(([cat, list]) => (
                  <optgroup key={cat} label={cat}>
                    {list.map((r) => (
                      <option key={r.name} value={r.name}>{r.name} (최소 {r.ilvl})</option>
                    ))}
                  </optgroup>
                ))}
              </select>
            ...
            <div style={{ marginBottom: 14 }}>
              <label style={{ fontSize: 13, color: "#888" }}>날짜 선택 (선택)</label>
              <input
                type="datetime-local"
                value={form.date}
        ...
        <div style={{ padding: "0 16px", maxWidth: 420 }}>
          ...
          {detailParty.groups.map((g, gi) => (
            <GroupCard
              ...
              isMaster={detailParty.masterId === myId}
          ...
          {detailParty.masterId !== myId && myStatus.status === "none" && (
